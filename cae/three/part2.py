class LineSegment:
    def __init__(self, x1, y1, x2, y2, currSteps):
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
        self.y2 = y2
...
class Wire:
    ...
    def addLine_Relative(self, point):
        ...
        self.addLine(LineSegment(startPoint[0], startPoint[1],
                                 newPoint[0], newPoint[1], self.currentSteps))

        self.currentLocation = newPoint
        self.currentSteps = self.currentSteps + max(abs(point[0]), abs(point[1]))
...
for i in wire1.lines:
    ...
    for j in wire2.lines:
        crossings = getLineIntersections(i,j)
        if len(crossings) > 0:
            dist = i.currentSteps + j.currentSteps
            dist = dist + distBtwn(i.point1, crossings[0])
            dist = dist + distBtwn(j.point1, crossings[0])
            intersectionDistances.append(dist)

if 0 in intersectionDistances:
    intersectionDistances.remove(0)
